...
class MultivariateNormal(Distribution):
    ...
    def __init__(self, loc, covariance_matrix=None, precision_matrix=None, scale_tril=None, validate_args=None):
        ...
        loc_tensor = loc_[..., 0]  # drop rightmost dim

        batch_shape, event_shape = loc_tensor.shape[:-1], loc_tensor.shape[-1:]
        super(MultivariateNormal, self).__init__(batch_shape, event_shape, validate_args=validate_args)

        if scale_tril is not None:
            _unbroadcasted_scale_tril_tensor = scale_tril
        elif covariance_matrix is not None:
            _unbroadcasted_scale_tril_tensor = torch.cholesky(covariance_matrix)
        else:  # precision_matrix is not None
            _unbroadcasted_scale_tril_tensor = _precision_to_scale_tril(precision_matrix)
        self.loc = torch.nn.Parameter(loc_tensor)
        self._unbroadcasted_scale_tril = torch.nn.Parameter(_unbroadcasted_scale_tril_tensor)
    ...
    def expand(self, batch_shape, _instance=None):
        ...
        new._unbroadcasted_scale_tril = torch.nn.Parameter(self._unbroadcasted_scale_tril.detach())
        if 'covariance_matrix' in self.__dict__:
            new.covariance_matrix = self.covariance_matrix.expand(cov_shape).detach()
        if 'scale_tril' in self.__dict__:
            new.scale_tril = self.scale_tril.expand(cov_shape).detach()
        if 'precision_matrix' in self.__dict__:
            new.precision_matrix = self.precision_matrix.expand(cov_shape).detach()
    ...
    def precision_matrix(self):
        precision = torch.cholesky_inverse(self._unbroadcasted_scale_tril,upper=False)
        return precision.expand(self._batch_shape + self._event_shape + self._event_shape)
    ...
    def lrsample(self, X): # samples that have same mean and var as _batch_mv(X,self.sample(...))
        Y_mean = torch.matmul(X,self.loc)
        eps = _standard_normal(Y_mean.size(), dtype=self.loc.dtype, device=self.loc.device)
        Y_sd = ((torch.matmul(X,self._unbroadcasted_scale_tril)**2).sum(-2)).sqrt()
        return Y_mean + Y_sd * eps
