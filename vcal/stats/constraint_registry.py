...
:class:`~.constraints.Constraint` objects to
:class:`~.transforms.Transform` objects. These objects both
input constraints and return transforms, but they have different guarantees on
bijectivity.

1. ``biject_to(constraint)`` looks up a bijective
   :class:`~.transforms.Transform` from ``constraints.real``
...
.. note::
    ...
    :class:`~.transforms.SoftmaxTransform` that simply
    exponentiates and normalizes its inputs; this is a cheap and mostly
    coordinate-wise operation appropriate for algorithms like SVI. In
    contrast, ``biject_to(constraints.simplex)`` returns a
    :class:`~.transforms.StickBreakingTransform` that
...
class ConstraintRegistry(object):
    ...
    def register(self, constraint, factory=None):
        """
        Registers a :class:`~.constraints.Constraint`
        ...
        Args:
            constraint (subclass of :class:`~.constraints.Constraint`):
                A subclass of :class:`~.constraints.Constraint`, or
                a singleton object of the desired class.
            factory (callable): A callable that inputs a constraint object and returns
                a  :class:`~.transforms.Transform` object.
        """
    ...
    def __call__(self, constraint):
        ...
        Args:
            constraint (:class:`~.constraints.Constraint`):
                A constraint object.
