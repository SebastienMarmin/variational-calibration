...
from .gaussian_matrix import GaussianMatrix
...
def register_kl(type_p, type_q):
    ...
    Args:
        type_p (type): A subclass of :class:`~.Distribution`.
        type_q (type): A subclass of :class:`~.Distribution`.
    """
...
def kl_divergence(p, q):
    ...
    Args:
        p (Distribution): A :class:`~.Distribution` object.
        q (Distribution): A :class:`~.Distribution` object.
...
def _L1_inverse_times_L0(L1,L0,transpose=False):# return L1^-1 L0 or L1^-1T L0^T
    if L1.diagonal:
        if L0.diagonal:
            return L0.parameter/L1.parameter, None
        else:
            if transpose:
                L0_data = L0.tril.transpose(-2,-1)
            else:
                L0_data = L0.tril
            return 1/L1.parameter.squeeze(-2).unsqueeze(-1)*L0_data, None
    else: # compute and store L1 inverse ast it will be useful twice
        L1_param = L1.parameter
        I = torch.eye(L1.d, dtype=L1_param.dtype,device=L1_param.device)
        if L1._homoscedastic:# rare case
            common_std = L1_param[...,0,0] # for benefiting from the unitriangular option
            L1_inverse = 1/common_std*torch.triangular_solve(I,L1_param/common_std, upper=False,transpose=transpose,unitriangular=True)[0]
        else:
            L1_inverse = torch.triangular_solve(I,L1_param,upper=False,transpose=transpose)[0]
            # torch.tril is done inside
        if L0.diagonal:
            return L1_inverse*L0.parameter, L1_inverse
        else:
            if transpose:
                L0_data = L0.tril.transpose(-2,-1)
            else:
                L0_data = L0.tril
            return torch.matmul(L1_inverse,L0_data), L1_inverse



@register_kl(GaussianMatrix, GaussianMatrix)
def _kl_matnorm_matnorm(p, q):
    # p ~ MN(M0,U0,V0), U0 = L0 L0^T, V0 = L0'^T L0'
    # q ~ MN(M1,U1,V1), U1 = L1 L1^T, V1 = L1'^T L1'
    # L and L' matrices are the lower triangular respectively in .row_scale.parameter and .row_scale.parameter
    # A formulation avoiding to do any Kronecker is used:
    # kl = 1/2 [ tr(V1^-1 V0)tr(U1^-1 U0)) + ||vect(L1^-1(M1-M0)L1^-1')||^2 - nrow ncol 
    #                       + nrow (logdet(V1)-logdet(V0)) + ncol (logdet(U1)-logdet(U0)) ]
    # No matrix inversion are performed if the corresponding covariance is diagonal.
    # This formula was obtained using
    #    - the link between Matrix normal, kronecker product ⊗ and multivariate normal,
    #    - Tr(A ⊗ B) = Tr(A)Tr(B)
    #    - det(A ⊗ B) = det(A)^dimB det(B)^dimA
    #    - (A ⊗ B)vect(C) = vect(BCA^T)
    #    - ...
    U0 = p.row_cov
    V0 = p.col_cov
    U1 = q.row_cov
    ...
    L1_inverseL0,  L1_inverse   = _L1_inverse_times_L0(U1,U0,transpose=False)
    Lp1_inverseLp0,L1p_inverseT  = _L1_inverse_times_L0(V1,V0,transpose=True)

    term1 = (L1_inverseL0**2).sum([-1,-2])*(Lp1_inverseLp0**2).sum([-1,-2])
    if p.centered and q.centered:
        term2 = 0
    else:
        if L1_inverse is None:
            L1_inverse_M = U1.L_inverse_times(p.loc-q.loc)
        else:
            L1_inverse_M = torch.matmul(L1_inverse,p.loc-q.loc)
        if L1p_inverseT is None:
            mat_term2 = V1.L_inverse_times(L1_inverse_M.transpose(-2,-1),transpose=True)
        else:
            mat_term2 = torch.matmul(L1p_inverseT,L1_inverse_M.transpose(-2,-1))
        term2 = ((mat_term2)**2).sum([-2,-1])
    term3 = n*(V1.log_det-V0.log_det)+m*(U1.log_det-U0.log_det)
    #    using Tr(U1^-1 U0) = FroebeniusNorm(L1^-1 L0)**2
    
    return 0.5*(term1+term2+term3-n*m)
