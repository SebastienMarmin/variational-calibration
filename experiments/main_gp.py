...
if __name__ == '__main__':
    ...
    model.likelihood.row_cov.parameter.data = (((scale_factor*args.noise_std))*torch.ones(1,1)).to(model.likelihood.row_cov.parameter.device)
    ...
    gp.prior_variances.data =  gp.pf(scale_factor**2)
    gp.lengthscales.data = gp.pf(0.05*ones_like(gp.lengthscales))
    #gp.full_cov_W = True
    ...
    model.likelihood.row_cov.parameter.require_grad = True # TODO user friendly
    ...
    #plt.close()
    print(model.likelihood.row_cov)
    print(model.likelihood.col_cov)
    print(model.likelihood.row_cov.variance)
    gp.set_to_posterior(X,Y,noise_covariance=model.likelihood.row_cov.variance)
